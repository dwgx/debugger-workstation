...
| `docs/BOOTSTRAP.md` | InstallRoot copy + project MCP (router-only vs inventory) |
...
| `skills/debugger-review/` | Dual-axis review + learning loop for agents. |
| `notes/` | Short durable facts (no samples, no secrets). |
...
| `scripts/download-tools.ps1` | Optional official GitHub/direct downloads (`-Apply`). Npcap installer is fetched, not installed. |
| `scripts/restore-from-cache.ps1` | Unpack an optional **private** zip cache you maintain yourself ([docs/CACHE.md](docs/CACHE.md)). Not required for a public clone. |
...
# 3. download tool binaries (authorized; uses gh when available)
...
Only `debugger-router` — one lightweight MCP — is enabled by default. It spins up whichever of the 19 backends is needed, on demand, and exits when done. Do not load all backends at once by default. For direct connections, use a profile (`mcp-mobile` / `mcp-re` / `mcp-net` / `mcp-ce` / `mcp-intel` / `mcp-all`).

## Security posture

The toolset includes debuggers, injectors, hooks, Frida, packet capture, and memory-analysis tools that may trigger AV/EDR. For high-risk operations (installing drivers/services/Npcap, modifying registry/Defender/startup, running untrusted samples on the host), the AI will brief you on consequences and rollback before proceeding — it won't silently act or refuse. Intended for **authorized** security testing, CTFs, research, and education.
