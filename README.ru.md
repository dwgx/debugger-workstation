...
По умолчанию включён только `debugger-router` — один лёгкий MCP. Он по требованию поднимает тот из 15 бэкендов, который нужен, и завершается по окончании работы. Не загружайте все бэкенды сразу по умолчанию. Для прямых подключений используйте профиль (`mcp-mobile` / `mcp-re` / `mcp-net` / `mcp-ce` / `mcp-all`).
