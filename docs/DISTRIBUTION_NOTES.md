...
默认推荐只启用 `debugger-router`，它会按需临时启动后端 MCP。不要默认一次性加载全部 19 个后端，除非你明确要做全量 MCP 调试。
...
这些工具包含调试器、注入器、Hook、Frida、反反调试插件、抓包工具和内存分析工具，可能被安全软件拦截。在宿主机运行可疑样本前建议考虑 VM/沙箱隔离，用户决定后按意图执行。

安装驱动、服务、Npcap、Defender 例外、计划任务或注册表项等系统级变更，AI 应先简要说明后果与回滚方式，用户确认后执行。
