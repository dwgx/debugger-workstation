...
The public GitHub history is a **reference skeleton**. The product for an agent is **this working copy**. The clone owner already has their own tools and prompts; this tree is the contract they chose to keep next to those tools.
...
2. Explore (read-only): manifests, docs, skills, `git status`.
3. Plan the files you will edit. Wait if the change amends a **stop line**.
4. Patch. Keep section order and i18n siblings in sync (`docs/I18N.md`).
5. Dual-axis review (`skills/debugger-review`). Evidence = commands that ran.
...
`OWNER.md`, `local.json`, `LOCAL-THIS-PC.md`, `.env`, binaries, samples, dumps, SDK cookies. See `.gitignore` and `DISCLAIMER.md`.

## Upstream vs this fork

Publishing to `dwgx/debugger-workstation` is optional and only when this clone's `origin` is that repo **and** the owner asked. Other remotes: follow the owner.
